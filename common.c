...
{
    printf("minicc - Mini C Compiler\n");
    printf("Team members:\n");
    printf("Emmanuel Pons - Ali Soukarieh\n");
}
...
static void print_help(const char *prog_name)
{
    ...
    printf("Options:\n");
    printf("  -b            Display banner (compiler name + team members)\n");
    printf("  -o <filename> Output assembly file (default: out.s)\n");
    printf("  -t <int>      Trace level 0-5 (default: 0)\n");
    printf("  -r <int>      Max registers 4-8 (default: 8)\n");
    printf("  -s            Stop after syntax analysis\n");
    printf("  -v            Stop after verification (passe_1)\n");
...
void parse_args(int argc, char **argv)
{
    ...
    // Validation: -b must be used alone
    if (banner)
    {
        if (argc != 2)
        {
            fprintf(stderr, "Error: -b option must be used alone\n");
            exit(1);
        }
        print_banner();
        exit(0);
    ...
    // Validation: -s and -v are mutually exclusive
    if (stop_after_syntax && stop_after_verif)
    {
        fprintf(stderr, "Error: -s and -v options are mutually exclusive\n");
        exit(1);
    }

    // Set max registers if specified
    if (max_reg_set)
    {
        set_max_registers(max_reg);
    }
